...
//const salida = document.getElementById('salida')
   salida.addEventListener('click', remove) 
...
function remove(e) {
  console.log('Diste click en borrar remove')
	var target = e.target;
                                      //usa la clase ('removeTast') como un selector
	if (target.classList.contains('borrarp')) {
		salida.removeChild(target.parentNode);
		var task = target.previousSibling.data;

		//funcion remover de LS
...
function localStorageListo() {
 ...
 function borrarx( list){

let lists
  
  lists = obtenerEntradaLS()

  lists.forEach(function(list, index){
   localStorage.removeItem(lists, index)
  })
